package br.com.lib02;

import java.util.Scanner;

public class Teste {
	public static void main(String[] args) {

		/* Exercicio 05 */

//	       String texto = "Texto com voga"
//	       		+ "is";
//	        int numVogais = Lib02Ex05.contarVogais(texto);
//
//	        System.out.println("O texto \"" + texto + "\" possui " + numVogais 
//	        		+ " vogais.");

//	        System.out.println();

		/* Exercicio 06 */
//	        String camelCase = "TextoEmCamelCase.";
//	        String textoConvertido = Lib02Ex06.converterCamelCase(camelCase);
//
//	        System.out.println("Texto em CamelCase: " + camelCase);
//	        System.out.println("Texto convertido: " + textoConvertido);
//	        
//	        System.out.println();

		/* Exercicio 07 */
//			Scanner input = new Scanner (System.in);
//			
//			System.out.println("Digite o texto original:");
//			String textoOriginal = input.nextLine();
//			
//			System.out.println("Digite a palavra que quer alterar:");
//			String palavraOriginal = input.nextLine();
//			
//			System.out.println("Digite a nova palavra:");
//			String novaPalavra = input.nextLine();
//			input.close();
//			String textoSubstituido = Lib02Ex07.substituirPalavra(textoOriginal, palavraOriginal, novaPalavra);
//
//			System.out.println("Texto original: " + textoOriginal);
//			System.out.println("Palavra a ser substituída: " + palavraOriginal);
//			System.out.println("Nova palavra: " + novaPalavra);
//			System.out.println("Texto com substituição: " + textoSubstituido);

//			System.out.println();

		/* Exercicio 08 */
//			Scanner input = new Scanner (System.in);
//			
//			System.out.println("Digite o primeiro número:");
//			int num1 = input.nextInt();
//			
//			System.out.println("Digite o segundo número:");
//			int num2 = input.nextInt();
//			input.close();
//			
//			System.out.println("O maior número é: "+Lib02Ex08.encontrarMaiorNumero(num1, num2));

		/* Exercicio 09 */
		int i = 1;
		int soma = 0;
		while (i <= 100) {
			if (i % 2 != 0) {
				soma += i;
			}
			i++;
			System.out.println(soma);
		}
	}
}
